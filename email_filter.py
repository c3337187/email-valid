...
Особенности:
- запуск без параметров — берёт `emails_input.txt` рядом с exe/скриптом;
    ...
    with path.open("w", encoding="utf-8") as handle:
        for result in results:
            if result.status == "valid":
                ...
                handle.write(f"{result.email} ; {result.reason}\n")
...
def ensure_input_file(path: Path) -> None:
    ...
    logging.info("Создан шаблон входного файла: %s", path)
...
def parse_args(base_dir: Path) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Input CSV or TXT file (default: emails_input.txt рядом с exe)",
    )
    parser.add_argument(
        "--valid-output",
        type=Path,
...
def main() -> None:
    ...
    for email in unique_emails:
        ...
        if result.status == "valid":
            ...
            logging.error("Проверка адреса: %s - %s", result.email, result.reason)
